...
object BfSearch {
  ...
  def bfExplore[A](g: Graph[A], start: Node[A]): List[Node[A]] =
    bfSearch(g,
      BfAcc[A](
        ...
        Set(start),
        BfPath(Nil)))
      .res match { case BfPath(p) ⇒ p.reverse }
  ...
  def rememberPath[A](g: Graph[A], n: Node[A], path: List[Node[A]]): BfPath[A] = BfPath(n :: path) // O(1)
  def rememberDist[A](g: Graph[A], n1: Node[A], n2: Node[A], ds: Map[Node[A],Int]): BfDist[A] = BfDist(ds + ((n2,ds(n1) + 1))) // O(1)
